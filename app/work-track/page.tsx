...
const formatNYTime = (dateInput?: string | Date | null): string => {
  if (!dateInput) return "--:--";
...
// Get Date object in NY timezone for elapsed calculation
const getNYDate = (dateInput?: string | Date | null): Date => {
...
export default function WorkTrack() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setTodayData(data);
      if (data?.checkIn) {
        setCheckInTime(formatNYTime(data.checkIn));
        setCheckedIn(!data.checkOut);
        setIsCompleted(!!data.checkOut);
      }
    }
  ...
  // Elapsed Timer - Now uses New York Time
  useEffect(() => {
    if (!checkedIn || !checkInTime) return;

    const interval = setInterval(() => {
      const start = getNYDate(todayData?.checkIn || checkInTime);
      const diff = Math.floor((Date.now() - start.getTime()) / 1000);
    ...
    return () => clearInterval(interval);
  }, [checkedIn, checkInTime, todayData?.checkIn]);
  ...
  const formatTime = (date: string | null) => formatNYTime(date);
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      const res = await dispatch(checkIn("")).unwrap();
      setCheckedIn(true);
      setCheckInTime(formatNYTime(res.checkIn));
      setTodayData(res);
      startTracking();
    } catch (err) {
      console.log(err);
